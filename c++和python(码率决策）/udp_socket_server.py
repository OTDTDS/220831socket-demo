...
def main():
    ...
    while True:  # 服务器无线循环
        # 3 调用xx程序,数据由server
        # agent(agent_id, server_sock, net_params_queue, exp_queue)##从server接收数据,类似数据容器
        # 4 定义包类型
        instanceId_this = -1  # not a valid instanceId
        min_delay = 50  # ms
        end_of_video = 0

        Head = struct.Struct('HIq')  # 离开包
        handshake_payload_fmt = struct.Struct('HIqIII')  # 握手包
        payload_fmt = struct.Struct('HIqffffff?')  # 离开包+数据包
        ...
        # 6 获得MsgType和instanceid
        headdata = recv_str[0:24]
        head = Head.unpack(headdata)
        # a,b,c,d,e,f= struct.unpack('HTqIII', recv_str)
        MsgType = head[0]
        instanceId = head[1]

        #根据MsgType 判断数据包
        if MsgType == 0:  # headsnake
            data = handshake_payload_fmt.unpack(recv_str)
            MIN_BIT_RATE = data[3]
            MAX_BIT_RATE = data[4]
            DEFAULT_BIT_RATE = data[5]
            if instanceId not in instanceId_list:
                instanceId_this = instanceId
                instanceId_list.append(instanceId)
        elif MsgType == 9999:  # 离开会话
            if instanceId != instanceId_this:
                continue
            end_of_video = 1
            if instanceId in instanceId_list:
                instanceId_list.remove(instanceId)
        elif MsgType == 1:  # RL消息,+一个iter_count
            if instanceId != instanceId_this:
                continue
            data = payload_fmt.unpack(recv_str)
            sending_bit_rate = float(data[3])
            received_bit_rate = float(data[4])
            buffer_size = float(data[5])
            delay = float(data[6])
            packet_loss_rate = float(data[7])
            nack_sent_count = float(data[8])
            print('return_bit_rate:' + str(sending_bit_rate) + '\n'
