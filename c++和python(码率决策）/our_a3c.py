...
def build_summaries():
    td_loss = tf.Variable(0.)
    tf.summary.scalar("TD_loss", td_loss)#优势函数估计
    avg_reward = tf.Variable(0.)
    tf.summary.scalar("Avg_reward", avg_reward)#平均奖励
    # min_reward = tf.Variable(0.)
    # tf.summary.scalar("Min_reward", min_reward)
    # avg_entropy = tf.Variable(0.)
    # tf.summary.scalar("Avg_entropy", avg_entropy)#策略平均信息熵

    # summary_vars = [td_loss, avg_reward, min_reward, avg_entropy]
    summary_vars = [td_loss, avg_reward]
    summary_ops = tf.summary.merge_all()
